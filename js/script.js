	...
	function startGame(){
		...
		for(var i = 0; i < 40; i++){
			...
			//posiciona as cartas no tabuleiro
					...
					card.style.left = (i % 10 === 0) ? 5 + "px" : (i % 10) * 110 + 5 + "px";
					card.style.top = i < 10 ? 5 + "px" : Math.floor( i / 10 ) * 110 + 5 + "px";
		...
		if(flippedCards.length < 2){
			...
			if(flippedCards.length === 2){
				...
				if(flippedCards[0].childNodes[3].id === flippedCards[1].childNodes[3].id){
					...
					//verifica se o contador de acertos chegou a 20
					if(matches >= 20){
						//caso haja 20 acertos, chama a função que finaliza o jogo
						gameOver();
					}
				}
			}
